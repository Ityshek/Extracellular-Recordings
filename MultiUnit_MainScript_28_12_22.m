...
if answer{1} == '2'
    ...
    dlgtitle = 'Input';
    dims = [1 35];
    definput1 = {'2','0'}; definptOut = {'70'};
    answer1 = inputdlg(prompt,dlgtitle,dims,definput1);
end
for c = 1:FigPlotNum
    chan = {num2str(str2double(answer{3})-1+c)};
    ...
    if ~channelflag
        ...
        title(['Channel ',num2str(RC)])
        ylim([0 max(Psth)+0.1]); % xlim([-10 990]);
...
for c = 1:length(ActiveChannels)
    ...
    % PSTH Sorted
    for i=1:Data.dim{ActiveChannels(c)}
        [PSTH,binsize_sec,Psth_sort(i,:),Data.SpikeCount{i},label,window]=Build_psth(Rast_sort{i},sampling_freq);
        t_pst=1000*linspace(-10*10^-3,size(Psth_sort(i,:),2)*binsize_sec-30*10^-3,length(Psth_sort(i,:)));
...
[Data] = NaturalIntensityCalc(Data,RateCalcWindow,fname,window,Psth_sort(Data.Clusters,:),sampling_freq,SponFlag2);
%% Prosthetic Intensity Response Curve
RateCalcWindow = 10; % PSTH Bin Size in ms
[Data] = ProstheticIntensityCalc(Data,RateCalcWindow,fname,window,Psth_sort(Data.Clusters,:));

%% Save Data variable
% Change "SavePath" to the desiered folder
SavePath ='C:\Users\Itay\Desktop\Yossi Mandel Lab\Extracellular Recordings\Results\Data\Prosthetic\';
...
Ch = answer{3};
save([SavePath,'ProstheticFullFlash_Loc',Data.Loc,'_','Ch',Ch,'_',Data.StimDur,'ms_',stimfreq,'Hz_',date,'.mat'],"Data");
