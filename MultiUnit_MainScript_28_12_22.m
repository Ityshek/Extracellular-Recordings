...
for c=1:length(ActiveChannels)
    ...
    plot(t,raw_data{ActiveChannels(c)},'b',t,stim,'vg',t,threshold{ActiveChannels(c)},'r')
    ...
    for i=1:Data.dim{ActiveChannels(c)}
        ...
        for k = 1:length(Aligned_idx)
            if  Data.ClusterIdx{ActiveChannels(c)}(k) == i
                ...
                Data.ClusteredVal{i}(k) = raw_data{ActiveChannels(c)}(Aligned_idx(k));
            end
        end
        y{i} = nan(1,length(t));
...
%% Sorted Plots
for c = 1:length(ActiveChannels)
    [Data.Rast_sort,Data.ind_rast_sort]=build_rast_sort2(Data.ClusterIdx{ActiveChannels(c)},stimulus_indexes,sampling_freq,ind_rast,Data.dim{ActiveChannels(c)},stimulus_times);
    for i=1:Data.dim{ActiveChannels(c)}
...
[Data] = SponCalc(Data,sampling_freq,stimulus_indexes,window);
% 
...
SavePath ='C:\Users\Itay\Desktop\Yossi Mandel Lab\Extracellular Recordings\Results\Data\';
